/**
 * Script dynamique pour les tables de critères dans Claraverse - V14.2 (Isolation ultra-renforcée par Chat & FRAP avec hash spécifique)
 * @version 14.2.0
 * @description
 * - Isolation ultra-complète des données par chat et FRAP via inclusion renforcée du chatId, FRAP hash spécifique et clés de stockage.
 * - Hash spécifique pour FRAP pour éviter toute confusion entre instances similaires.
 ...
 * - Maintien de la persistance ultra-robuste avec signature DOM étendue.
...
(function () {
  ...
  console.log(
    "🚀 Initialisation V14.2 - Isolation ultra-renforcée par Chat & FRAP avec hash spécifique"
  );
  ...
  const CONFIG = {
    ...
    STORAGE_PREFIX: "claraverse_v14_",
  ...
  // ================================
  // SYSTÈME DE SIGNATURE DOM ULTRA-ROBUSTE AVEC HASH FRAP SPÉCIFIQUE
  // ================================
  const DOMSignature = {
    // Génère une signature ultra-précise d'une table Flowise avec hash FRAP spécifique
    generateTableSignature(table) {
      ...
      // Signature basée sur le contenu COMPLET de la div, FRAP et chatId
      const signature = {
        ...
        frapHash: null, // Ajout pour isolation FRAP
      };

      // Isolation renforcée pour FRAP : Hash du texte exact de la cellule contenant FRAP
      if (signature.keyword === "frap") {
        const allCells = table.querySelectorAll("td, th");
        for (const cell of allCells) {
          const cellText = cell.textContent.trim().toLowerCase();
          if (
            CONFIG.SEARCH_KEYWORDS.frap.some((kw) =>
              cellText.includes(kw.toLowerCase())
            )
          ) {
            signature.frapHash = this.hashString(cell.textContent.trim());
            break;
          }
        }
      }
    ...
    createSignatureString(signature) {
      return `sig_${signature.chatId}_${signature.keyword}_${signature.divTextHash}_${signature.divIndex}_${signature.tableCount}_${signature.frapHash || ""}`;
    },
  };

  // ================================
  // GESTIONNAIRE DE PERSISTANCE ULTRA-ROBUSTE (ISOLÉ PAR CHAT ET FRAP AVEC HASH)
  // ================================
  const PersistenceManager = {
    ...
    cleanup() {
      ...
      if (cleaned > 0) {
        console.log(
          `🧹 Nettoyage: ${cleaned} enregistrements supprimés (anciens ou autres chats/FRAP)`
        );
      }
    },
  };
  ...
  function initialize() {
    ...
    console.log(
      "✅ V14.2 initialisée - Isolation par chat et FRAP ultra-renforcée active"
    );
  }
  ...
  // API de débogage
  window.ClaraverseV14Debug = {
    DOMSignature,
    PersistenceManager,
    scanAndProcessAllTables,
    CONFIG,
    version:
      "14.2.0 - Isolation ultra-renforcée par Chat & FRAP avec hash spécifique",
  };
})();
