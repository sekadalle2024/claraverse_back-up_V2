...
export class LocalStorageDB {
  ...
  async getRecentChats(limit: number = 10): Promise<Chat[]> {
    ...
    if (this.useIndexedDB) {
      ...
      return chats
        .filter(chat => chat.user_id === currentUserId)
  ...
  async getChatMessages(chatId: string): Promise<ChatMessage[]> {
    ...
    if (this.useIndexedDB) {
      const allMessages = await indexedDBService.getAll<ChatMessage>('messages');
      return allMessages
        .filter(message => message.chat_id === chatId && message.user_id === currentUserId)
  ...
  // Storage methods
  async addStorageItem(item: Omit<StorageItem, 'id' | 'timestamp' | 'user_id'>): Promise<string> {
    const currentUserId = await this.getCurrentUser();
    if (!currentUserId) {
      throw new Error('No user logged in');
    }
  ...
  async getAllUsers(): Promise<User[]> {
    console.log('🔍 DatabaseService: getAllUsers called');
    if (this.useIndexedDB) {
      console.log('📊 DatabaseService: Using IndexedDB to get all users');
      const users = await indexedDBService.getAll<User>('users') || [];
      console.log('👥 DatabaseService: Found users in IndexedDB:', users.length, users);
      return users;
    } else {
      console.log('📊 DatabaseService: Using localStorage to get all users');
      const users = await this.getItemFromLocalStorage<User[]>('users') || [];
      console.log('👥 DatabaseService: Found users in localStorage:', users.length, users);
      return users;
    }
  }
  ...
  async getUser(id: string): Promise<User | null> {
    if (this.useIndexedDB) {
      const user = await indexedDBService.get<User>('users', id);
      return user || null;
    } else {
  ...
  async createUser(userData: {
    ...
    console.log('👤 DatabaseService: Creating user:', { id: userId, name: userData.name, email: userData.email });

    if (this.useIndexedDB) {
      await indexedDBService.put('users', newUser);
      console.log('✅ DatabaseService: User saved to IndexedDB');
    } else {
      const users = await this.getAllUsers();
      users.push(newUser);
      await this.setItemToLocalStorage('users', users);
      console.log('✅ DatabaseService: User saved to localStorage');
    }
  ...
  async updateUser(id: string, updates: Partial<User>): Promise<void> {
    if (this.useIndexedDB) {
      ...
      if (index !== -1) {
        ...
        this.setItemToLocalStorage('users', users);
      }
    }
  }
  ...
  async deleteUser(id: string): Promise<void> {
    if (this.useIndexedDB) {
      ...
      this.setItemToLocalStorage('users', filteredUsers);
    }
  }

  async getCurrentUser(): Promise<string | null> {
    console.log('🔍 DatabaseService: getCurrentUser called');
    if (this.useIndexedDB) {
      const result = await indexedDBService.get<{key: string, value: string}>('settings', 'current_user');
      const userId = result ? result.value : null;
      console.log('📊 DatabaseService: Current user from IndexedDB:', userId);
      return userId;
    } else {
      const userId = localStorage.getItem('current_user');
      console.log('📊 DatabaseService: Current user from localStorage:', userId);
      return userId;
    }
  }
  ...
  async setCurrentUser(userId: string): Promise<void> {
    console.log('💾 DatabaseService: Setting current user to:', userId);
    if (this.useIndexedDB) {
      await indexedDBService.put('settings', { key: 'current_user', value: userId });
      console.log('✅ DatabaseService: Current user saved to IndexedDB');
    } else {
      localStorage.setItem('current_user', userId);
      console.log('✅ DatabaseService: Current user saved to localStorage');
    }
  }
  ...
  async clearCurrentUser(): Promise<void> {
    if (this.useIndexedDB) {
      ...
      localStorage.removeItem(`${DB_PREFIX}current_user_id`);
    }
  }
}
