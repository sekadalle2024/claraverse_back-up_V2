...
export class ClaraApiService {
  ...
  private flowiseEndpoint =
    "https://r534c2br.rpcld.co/api/v1/prediction/6a5889b3-845a-47fc-96db-5796f323c263";
  ...
  public async sendChatMessage(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Flowise API error: ${response.status} ${response.statusText}`
        );
      }
      ...
      const claraMessage: ClaraMessage = {
        ...
        role: "assistant",
        content:
          flowiseResponseContent ||
          "I apologize, but I was unable to get a response from Flowise.",
        timestamp: new Date(),
        metadata: {
          model: "Flowise",
      ...
      return {
        ...
        role: "assistant",
        content:
          "I apologize, but I encountered an error while processing your request with Flowise. Please try again.",
        timestamp: new Date(),
        metadata: {
          error:
    ...
    if (
      ...
      try {
        ...
        if (requestedProvider) {
          ...
          console.log(
            `🚀 Switched to provider: ${requestedProvider.name} (${requestedProvider.baseUrl})`
          );
        } else {
          throw new Error(
            `Provider ${config.provider} not found or not configured`
          );
        }
      } catch (error) {
        console.error(
        ...
        throw new Error(
          `Failed to switch to provider ${config.provider}: ${error instanceof Error ? error.message : "Unknown error"}`
        );
      }
    } else if (config.provider) {
      console.log(
        `✅ Already using correct provider: ${currentProvider?.name} (${currentProvider?.baseUrl})`
      );
    }
  }
