/**
 * Script dynamique pour les tables de critères dans Claraverse - V14.3 (Inclusion table déclencheuse)
 * @version 14.3.0
 * @description
 ...
 * - Envoie les données HTML consolidées à l'endpoint n8n.
 ...
 * - NOUVEAU V14.3: Inclusion de la table déclencheuse dans l'envoi vers l'endpoint n8n.
 */
(function () {
    "use strict";
  
    console.log(
      "🚀 Initialisation du script dynamique de tables V14.3 (Inclusion table déclencheuse - n8n)"
    );
  
    // --- CONFIGURATION CENTRALE ---
    const CONFIG = {
      N8N_ENDPOINT_URL: "https://pq5ciwuv.rcld.app/webhook/template",
    ...
    /**
     * Interroge l'endpoint n8n avec les données collectées.
    ...
    async function queryN8nEndpoint(tablesHTML, targetKeyword) {
      try {
        ...
        console.log(
          `✅ Données reçues de l'endpoint n8n ! Statut: ${response.status} OK`
        );
    ...
    /**
     * Détecte le mot-clé cible [critère table cible] dans une table "n8n".
     * @param {HTMLElement} n8nTable - L'élément de la table déclencheuse.
     ...
     * @param {HTMLElement} triggerTable - La table déclencheuse à inclure en fin de collecte.
     * @returns {string} Une chaîne HTML contenant toutes les tables trouvées + la table déclencheuse.
     */
    function collectCriteriaTables(targetKeyword, triggerTable = null) {
      ...
      // Ajouter la table déclencheuse à la fin si elle est fournie
      if (triggerTable) {
        console.log(
          `📋 Ajout de la table déclencheuse pour le mot-clé "${targetKeyword}" à la fin de la collecte`
        );
      ...
      console.log(
        `📊 Collecte terminée: ${totalTableCount} table(s) au total (incluant la table déclencheuse si applicable)`
      );
    ...
    /**
     * Extrait les tables HTML du texte de réponse n8n (format markdown ou HTML).
    ...
    /**
     * Trouve le conteneur cible où insérer les tables n8n (la div contenant la table déclencheuse).
     ...
     * @param {HTMLElement[]} n8nTables - Les tables intégrées
     */
    ...
    /**
     * Intègre les tables n8n dans le DOM avec un formatage CSS correct.
     * @param {HTMLElement[]} n8nTables - Les tables à insérer.
    ...
    /**
     * Orchestre le processus complet pour une table "n8n" détectée.
     * @param {HTMLElement} triggerTable - La table qui a déclenché le processus.
     */
    async function processN8nTrigger(triggerTable) {
      ...
      try {
        // 1. Collecter les tables (incluant la table déclencheuse)
        const criteriaTablesHTML = collectCriteriaTables(targetKeyword, triggerTable);
        ...
        // 2. Envoyer à l'endpoint n8n (avec cache)
    ...
    /**
     * Scanne le document à la recherche de tables "n8n" non traitées.
     */
    function scanAndProcess() {
      ...
      if (processedCount > 0) {
        console.log(
          `🔍 Scanner: ${processedCount} nouvelles tables n8n détectées`
        );
      }
    }
     ...
     */
    function initialize() {
      console.log("🔧 Initialisation du script V14.3...");
      ...
      console.log(
        "✅ Script V14.3 initialisé - CSS, affichage des tables, persistance des données et inclusion table déclencheuse avec n8n"
      );
    }
    ...
    window.ClaraverseN8nV14_3 = {
      ...
      version: "14.3.0 - Inclusion table déclencheuse + Persistance des données avec n8n",
    };
  })();